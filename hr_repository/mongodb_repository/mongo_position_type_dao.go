...
func (p *PositionTypeMongoDBDao) InitializeDao(client utils.Map, businessId string) {
	log.Println("Initialize PositionType Mongodb DAO")
	p.client = client
...
func (p *PositionTypeMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", hr_common.DbHrPositionTypes)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrPositionTypes)
	if err != nil {
		return nil, err
	}
...
func (p *PositionTypeMongoDBDao) Get(account_id string) (utils.Map, error) {
	...
	filter := bson.D{
		{Key: hr_common.FLD_POSITION_TYPE_ID, Value: account_id},
		{Key: hr_common.FLD_BUSINESS_ID, Value: p.businessID},
		{Key: db_common.FLD_IS_DELETED, Value: false}}
...
func (p *PositionTypeMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Business PositionType Save - Begin", indata)
	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrPositionTypes)
	if err != nil {
		return indata, err
	}
	...
	log.Println("Save - End", indata[hr_common.FLD_POSITION_TYPE_ID])
