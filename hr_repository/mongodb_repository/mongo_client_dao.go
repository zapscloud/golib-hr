...
func (p *ClientMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", hr_common.DbHrClients)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrClients)
	if err != nil {
		return nil, err
	}
...
func (p *ClientMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Save - End", indata[hr_common.FLD_CLIENT_ID])
...
func (p *ClientMongoDBDao) appendListLookups(stages []bson.M) []bson.M {
