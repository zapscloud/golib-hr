...
func (p *DepartmentMongoDBDao) InitializeDao(client utils.Map, businessId string) {
	log.Println("Initialize Department Mongodb DAO")
	p.client = client
...
func (p *DepartmentMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", hr_common.DbHrDepartments)
...
func (p *DepartmentMongoDBDao) GetDeptCodeDetails(departmentcode string) (utils.Map, error) {
	...
	log.Println("DepartmentMongoDBDao::Get:: Begin ", departmentcode)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrDepartments)
	log.Println("Find:: Got Collection ")

	filter := bson.D{{Key: "department_code", Value: departmentcode}, {}}
	...
	log.Println("DepartmentMongoDBDao::Get:: End Found a single document: \n", err)
	return result, nil
}
...
func (p *DepartmentMongoDBDao) Get(departmentid string) (utils.Map, error) {
	...
	log.Println("DepartmentMongoDBDao::Get:: Begin ", departmentid)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrDepartments)
	log.Println("Find:: Got Collection ")

	filter := bson.D{{Key: hr_common.FLD_DEPARTMENT_ID, Value: departmentid}, {}}
...
func (p *DepartmentMongoDBDao) Find(filter string) (utils.Map, error) {
	...
	log.Println("DepartmentMongoDBDao::Find:: Begin ", filter)
	...
	log.Println("DepartmentMongoDBDao::Find:: End Found a single document: \n", err)
	return result, nil
}
...
func (p *DepartmentMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Business Department Save - Begin", indata)
	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrDepartments)
	if err != nil {
	...
	log.Println("Save - End", indata[hr_common.FLD_DEPARTMENT_ID])
...
func (p *DepartmentMongoDBDao) Delete(departmentid string) (int64, error) {
	...
	log.Println("DepartmentMongoDBDao::Delete - Begin ", departmentid)
	...
	log.Printf("DepartmentMongoDBDao::Delete - End deleted %v documents\n", res.DeletedCount)
...
func (p *DepartmentMongoDBDao) DeleteAll() (int64, error) {
	...
	log.Println("DepartmentMongoDBDao::DeleteAll - Begin ")
