...
func (p *PositionMongoDBDao) InitializeDao(client utils.Map, businessId string) {
	log.Println("Initialize Position Mongodb DAO")
	p.client = client
...
func (p *PositionMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", hr_common.DbHrPositions)
...
func (p *PositionMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Business Position Save - Begin", indata)
	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, hr_common.DbHrPositions)
	if err != nil {
		return indata, err
	}
	...
	log.Println("Save - End", indata[hr_common.FLD_POSITION_ID])
